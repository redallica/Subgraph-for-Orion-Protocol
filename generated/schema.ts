...
export class TokenHolderCounter extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id !== null, "Cannot save TokenHolderCounter entity without an ID");
    assert(
      id.kind == ValueKind.STRING,
      "Cannot save TokenHolderCounter entity with non-string ID. " +
        'Considering using .toHex() to convert the "id" to a string.'
    );
    store.set("TokenHolderCounter", id.toString(), this);
  }

  static load(id: string): TokenHolderCounter | null {
    return store.get("TokenHolderCounter", id) as TokenHolderCounter | null;
  }
